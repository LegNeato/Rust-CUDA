...
struct Opt {
    ...
    /// The environment to compile to the CUDA tests.
    #[arg(long, default_value = "nvptx64")]
    target_env: String,
...
impl DepKind {
    ...
    fn target_dir_suffix(self, target: &str) -> String {
        match self {
            Self::CudaLib => format!("{target}/debug/deps"),
...
fn main() {
    ...
    // Pull in rustc_codegen_nvvm as a dynamic library in the same way
    // cuda-builder does.
    let codegen_backend_path = find_rustc_codegen_nvvm();
...
impl Runner {
    ...
    fn run_mode(&self, mode: &'static str) {
            ...
            .flat_map(|env| VARIATIONS.iter().map(move |variation| (env, variation)))
        {
            ...
            let target = format!("{env}{CUDA_TARGET_SUFFIX}");
            ...
            let mut flags = test_rustc_flags(
                ...
                &[
                    &self
                        .deps_target_dir
                        .join(DepKind::CudaLib.target_dir_suffix(&target)),
                    &self
                        .deps_target_dir
